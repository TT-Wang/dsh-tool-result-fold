    ...
    return defineTool({
        ...
        execute: async (args, exec) => {
            ...
            if (typeof a.grep === 'string' && a.grep.trim())
                return partialByGrep(hit.text, a.grep, head);
            if (typeof a.lines === 'string' && /^\d+\s*-\s*\d+$/.test(a.lines.trim()))
                return partialByLines(hit.text, a.lines, head);
    ...
    constructor(ctx, config = {}) {
        ...
        if (spillMin > 0) {
            ...
            ctx.on('tools/post-execute', async (exec, result, next) => {
                ...
                if (downstream.kind !== 'accept' || downstream.value !== undefined || downstream.content !== undefined)
                    return downstream;
                if (result.isError || exec.name === 'read' || exec.name === 'read_file' || exec.name === EXPAND_TOOL_NAME)
                    return downstream;
