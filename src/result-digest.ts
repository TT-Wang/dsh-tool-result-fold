...
// 列表项(`- `、`* `、`1. `)同理:文档里的废弃项、字段清单、变更记录都长这样;仍受块上限与键新颖性约束。
const STRUCTURED = /^\s*(?:[A-Za-z_][\w.\-/]*(?: [\w.\-/]+)?\s*[=:]\s*\S|#{1,6}\s|\[[^\]]+\]\s*$|```|\|.*\||[-*•]\s+\S|\d{1,3}[.)]\s+\S)/
...
export function digestToolResult(text: string, source: { tool: string; path?: string }, policy: DigestPolicy = DEFAULT_DIGEST_POLICY): DigestResult {
  if (source.tool === 'recall_step' || source.tool === 'recall_turn' || source.tool === 'recall_search' || source.tool === 'expand_result') return untouched(text, 'search')
  // grep/glob:每一行都是命中,小结果原样;巨量命中才按文件配额压(Headroom SearchCompressor)。
  if (source.tool === 'grep' || source.tool === 'glob') return digestSearch(text, policy)
  // 统一 diff(git diff / diff -u 的输出)按 hunk 压,不走代码不折的分支。
  if (looksLikeDiff(text)) return digestDiff(text, policy)
  // JSON / JSONL 工具结果(API 响应、清单):按元素压,而不是按行——美化 JSON 的每一行都像结构行,按行永远折不动。
  const json = tryJsonItems(text)
  if (json !== null) return digestJson(text, json, policy)
  ...
  if (kind === 'code') return untouched(text, kind)
  if (kind === 'search') return digestSearch(text, policy)
  if (kind === 'log') return digestLog(text, policy)
  return digestData(text, policy)
}

// ---------------------------------------------------------------- json digest(Headroom SmartCrusher 的简化版)

type JsonItems = { kind: 'array'; items: unknown[]; wrap: null } | { kind: 'jsonl'; items: unknown[]; wrap: null } | { kind: 'object'; items: unknown[]; wrap: { obj: Record<string, unknown>; key: string } }

/** 文本是 JSON 数组 / 含大数组字段的 JSON 对象 / JSONL 吗?是就给出元素列表。 */
export function tryJsonItems(text: string): JsonItems | null {
  const t = text.trim()
  if (t.startsWith('[') && t.endsWith(']')) {
    try { const v = JSON.parse(t); if (Array.isArray(v)) return { kind: 'array', items: v, wrap: null } } catch { /* not json */ }
    return null
  }
  if (t.startsWith('{') && t.endsWith('}')) {
    try {
      ...
      for (const [k, val] of Object.entries(v)) if (Array.isArray(val) && (best === null || val.length > best.len)) best = { key: k, len: val.length }
      if (best !== null && best.len >= 2) return { kind: 'object', items: v[best.key] as unknown[], wrap: { obj: v, key: best.key } }
    } catch { /* not json */ }
    return null
  }
  ...
  if (lines.length >= 3 && lines.every((l) => /^\s*[\[{]/.test(l))) {
    const items: unknown[] = []
    for (const l of lines) { try { items.push(JSON.parse(l)) } catch { return null } }
...
function itemLooksLikeError(item: unknown): boolean {
  if (typeof item !== 'object' || item === null) return false
  for (const [k, v] of Object.entries(item as Record<string, unknown>)) {
    if (ERRORISH.test(k) && v !== null && v !== false && v !== 0 && v !== '') return true
    if (typeof v === 'string' && v.length < 200 && ERRORISH.test(v)) return true
    if ((k === 'status' || k === 'statusCode' || k === 'code') && typeof v === 'number' && v >= 400) return true
    if (k === 'level' && typeof v === 'string' && /error|fatal|warn/i.test(v)) return true
  }
  return false
}

export function digestJson(text: string, json: JsonItems, policy: DigestPolicy = DEFAULT_DIGEST_POLICY): DigestResult {
  const items = json.items
  const n = items.length
  const totalLines = text.split('\n').length
  if (text.length < policy.minChars || n < policy.jsonMinItems) return { text, digested: false, totalLines, keptLines: totalLines, kind: 'json' }
  // 去重(内容相同的元素只留第一个)
  const seen = new Set<string>()
  const uniq: number[] = []
  for (let i = 0; i < n; i += 1) { const k = JSON.stringify(items[i]); if (!seen.has(k)) { seen.add(k); uniq.push(i) } }
  const K = policy.jsonKeepItems
  const keep = new Set<number>()
  const head = Math.max(1, Math.ceil(K * 0.3))
  const tail = Math.max(1, Math.ceil(K * 0.15))
  for (const i of uniq.slice(0, head)) keep.add(i)
  for (const i of uniq.slice(-tail)) keep.add(i)
  for (const i of uniq) { if (keep.size >= K) break; if (itemLooksLikeError(items[i])) keep.add(i) }
  if (keep.size >= n) return { text, digested: false, totalLines, keptLines: totalLines, kind: 'json' }
  const fields = new Set<string>()
  for (const it of items) if (typeof it === 'object' && it !== null && !Array.isArray(it)) for (const k of Object.keys(it as object)) fields.add(k)
  ...
  const out: string[] = []
  let omitted = 0
  const flush = (): void => { if (omitted > 0) { out.push(`…[+${omitted} of ${n} items omitted${dupNote}${fieldNote}]…`); omitted = 0 } }
  const sortedKeep = [...keep].sort((a, b) => a - b)
  let cursor = 0
  for (const i of sortedKeep) { omitted += i - cursor; flush(); out.push(JSON.stringify(items[i])); cursor = i + 1 }
  omitted += n - cursor; flush()
  let rendered: string
  if (json.kind === 'jsonl') rendered = out.join('\n')
  else if (json.kind === 'array') rendered = `[\n${out.join(',\n')}\n]`
  else {
    const rest: Record<string, unknown> = { ...json.wrap.obj }
    delete rest[json.wrap.key]
    rendered = `${JSON.stringify(rest)}\n"${json.wrap.key}": [\n${out.join(',\n')}\n]`
  }
  if (rendered.length >= text.length * policy.maxKeepRatio) return { text, digested: false, totalLines, keptLines: totalLines, kind: 'json' }
...
export function digestSearch(text: string, policy: DigestPolicy = DEFAULT_DIGEST_POLICY): DigestResult {
  const lines = text.split('\n')
  const n = lines.length
  const byFile = new Map<string, number[]>()
  for (let i = 0; i < n; i += 1) { const m = MATCH_LINE.exec(lines[i]!); if (m) { const arr = byFile.get(m[1]!) ?? []; arr.push(i); byFile.set(m[1]!, arr) } }
  const matches = [...byFile.values()].reduce((a, v) => a + v.length, 0)
  if (matches < policy.searchMinMatches && text.length < policy.searchMinChars) return untouched(text, 'search')
  if (matches === 0) return untouched(text, 'search')
  const keep = new Set<number>()
  for (let i = 0; i < n; i += 1) if (!MATCH_LINE.test(lines[i]!)) keep.add(i)   // 非命中行(标题、分隔)照留
  let total = 0
  ...
  for (const [file, idx] of byFile) {
    ...
    for (const i of pick) { if (total < policy.searchMaxTotal) { keep.add(i); total += 1 } }
    const kept = pick.filter((i) => keep.has(i)).length
    if (kept < idx.length) dropped.set(file, idx.length - kept)
  }
  if (dropped.size === 0) return untouched(text, 'search')
  const out: string[] = []
  const lastKeptOfFile = new Map<string, number>()
  for (const [file, idx] of byFile) for (const i of idx) if (keep.has(i)) lastKeptOfFile.set(file, i)
  for (let i = 0; i < n; i += 1) {
    if (!keep.has(i)) continue
    out.push(lines[i]!)
    const m = MATCH_LINE.exec(lines[i]!)
    if (m && lastKeptOfFile.get(m[1]!) === i && dropped.has(m[1]!)) out.push(`[... and ${dropped.get(m[1]!)} more matches in ${m[1]!}]`)
  }
  const rendered = out.join('\n')
  if (rendered.length >= text.length * policy.maxKeepRatio) return untouched(text, 'search')
...
export function digestDiff(text: string, policy: DigestPolicy = DEFAULT_DIGEST_POLICY): DigestResult {
  const lines = text.split('\n')
  const n = lines.length
  if (n < policy.diffMinLines) return { text, digested: false, totalLines: n, keptLines: n, kind: 'diff' }
  // 切 hunk:每个 @@ 到下一个 @@ / 文件头之前
  const keep = new Set<number>()
  const isHeader = (l: string): boolean => /^(?:diff --|index |--- |\+\+\+ |rename |similarity |dissimilarity |copy |new file|deleted file|old mode|new mode|Binary files)/.test(l)
  const hunkStarts: number[] = []
  for (let i = 0; i < n; i += 1) { if (isHeader(lines[i]!)) keep.add(i); if (lines[i]!.startsWith('@@ ')) hunkStarts.push(i) }
  // 文件头之前的内容(提交信息)原样
  const firstHeader = lines.findIndex((l) => isHeader(l) || l.startsWith('@@ '))
  for (let i = 0; i < Math.max(0, firstHeader); i += 1) keep.add(i)
  // 每个 hunk:头、改动行、改动两侧 diffContextLines 行、"\ No newline" 标记
  type Hunk = { start: number; end: number; changes: number; file: number }
  const hunks: Hunk[] = []
  let fileNo = -1
  for (let i = 0; i < n; i += 1) if (lines[i]!.startsWith('diff --')) fileNo += 1
  let fno = -1
  for (let h = 0; h < hunkStarts.length; h += 1) {
    const start = hunkStarts[h]!
    let end = h + 1 < hunkStarts.length ? hunkStarts[h + 1]! : n
    for (let i = start + 1; i < end; i += 1) if (isHeader(lines[i]!)) { end = i; break }
    while (fno + 1 <= fileNo && lines.slice(0, start).filter((l) => l.startsWith('diff --')).length > fno + 1) fno += 1
    let changes = 0
    for (let i = start + 1; i < end; i += 1) if (/^[+-]/.test(lines[i]!) && !/^(?:---|\+\+\+) /.test(lines[i]!)) changes += 1
    hunks.push({ start, end, changes, file: fno })
  }
  // 每文件 hunk 配额:首、末与改动最多的
  const byFile = new Map<number, Hunk[]>()
  for (const hk of hunks) { const arr = byFile.get(hk.file) ?? []; arr.push(hk); byFile.set(hk.file, arr) }
  const chosen = new Set<Hunk>()
  const droppedHunks = new Map<number, number>()
  for (const [file, arr] of byFile) {
    if (arr.length <= policy.diffMaxHunksPerFile) { for (const hk of arr) chosen.add(hk); continue }
    const pick = new Set<Hunk>([arr[0]!, arr[arr.length - 1]!])
    for (const hk of [...arr].sort((a, b) => b.changes - a.changes)) { if (pick.size >= policy.diffMaxHunksPerFile) break; pick.add(hk) }
    for (const hk of pick) chosen.add(hk)
  ...
  for (const hk of chosen) {
    ...
    for (let i = hk.start + 1; i < hk.end; i += 1) {
      ...
      if (/^[+-]/.test(l) || l.startsWith('\\ No newline')) {
        keep.add(i)
        for (let k = Math.max(hk.start + 1, i - policy.diffContextLines); k <= Math.min(hk.end - 1, i + policy.diffContextLines); k += 1) keep.add(k)
      }
    }
  }
  if (keep.size >= n) return { text, digested: false, totalLines: n, keptLines: n, kind: 'diff' }
  const out: string[] = []
  let i = 0
  while (i < n) {
    if (keep.has(i)) { out.push(lines[i]!); i += 1; continue }
  ...
  for (const [file, cnt] of droppedHunks) out.push(`[... and ${cnt} more hunks in file #${file + 1}]`)
  const rendered = out.join('\n')
  if (rendered.length >= text.length * policy.maxKeepRatio) return { text, digested: false, totalLines: n, keptLines: n, kind: 'diff' }
...
export function resolveDigestPolicy(input: Partial<DigestPolicy> | undefined): DigestPolicy {
  ...
  for (const k of ['jsonMinItems', 'jsonKeepItems', 'searchMinMatches', 'searchMinChars', 'searchMaxPerFile', 'searchMaxTotal', 'diffMinLines', 'diffContextLines', 'diffMaxHunksPerFile'] as const) if (!(p[k] >= 0)) throw new Error(`digest.${k} must be >= 0`)
