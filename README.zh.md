...
同日与纯原生 loop 对照(deepseek-v4-flash):

| 场景 | 原生 loop | 原生 loop + 折叠 | 说明 |
|---|---|---|---|
| 链式迁移,830K 字符工具输出(l1) | $0.135 | **$0.023** | 45/45 全对,峰值上下文 43K 对 331K token,46 次折叠 |
| 长会话记忆探针(s13) | $0.017 | $0.020 / $0.023 | 中性(没多少可折),探针全部保住 |
| 10 轮编码循环(s2) | $0.071 | $0.087 | 编码型一次没折;差在运行噪声(81 步对 68 步) |

结果大的任务(日志、文档、数据)是净收益,结果小的任务中性(源码、grep、短测试输出从不折)。
