...
| f9 文档站调研 | 9 页通过 `fetch_page` 整页拿,事实在表格、列表和散文里 | $0.0080 | $0.0068(−15%);有部分取回后 **$0.0063**(−21%) | 7 页,80K → 32K;模型取回了 3 页——`grep` 可用后 3 次都是部分取回 | 两臂 8/8 |
...
- 散文句子里的事实任何规则都留不住;`grep` 取回让补救变便宜,不是免费。折叠留下的是结构:键值行、标题、表格、列表、带上下文的错误行。f9 剩下的三次取回全是为了句子里的事实。
